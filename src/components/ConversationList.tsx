    ...
    () =>
      unique
        .filter((c) => {
          ...
          if (term) {
            const content =
              `${c.nome || ""} ${c.numero || ""} ${c.ultima_mensagem || ""}`.toLocaleLowerCase(
                "pt-BR"
              );
        ...
        .sort((a, b) => {
          ...
          return (
            new Date(b.ultima_atualizacao || 0).getTime() -
            new Date(a.ultima_atualizacao || 0).getTime()
          );
        }),
      ...
      {isLoading ? (
        <div className="px-3 py-2 space-y-2 animate-pulse" aria-label="Carregando conversas">
          {[1, 2, 3, 4].map((item) => (
            <div
              key={item}
              className="h-[106px] rounded-xl bg-slate-900 border border-white/[0.04]"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    ...
                    selectedIdLimpo === id && "bg-blue-500/10 border-blue-500/60"
                  )}
                >
