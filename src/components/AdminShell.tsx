    ...
    <div className="min-h-[100dvh] bg-[#070F1F] text-white">
      <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#020617]/95 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-3">
          ...
          <button type="button" onClick={() => navigate("/dashboard")} className="ml-auto hidden sm:flex items-center gap-2 text-xs text-slate-400 hover:text-white transition">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5 sm:py-8">
        <nav className="flex gap-1 p-1 rounded-xl bg-slate-950/70 border border-white/[0.06] mb-7 overflow-x-auto" aria-label="Seções administrativas">
          {navigation.map((item) => {
            const active = location.pathname === item.path;
            const Icon = item.icon;
            return (
              <button key={item.path} type="button" onClick={() => navigate(item.path)} className={cn("min-w-max flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition", active ? "bg-blue-600 text-white shadow-lg shadow-blue-950/30" : "text-slate-400 hover:bg-white/[0.05] hover:text-white")}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
          <div>
            <p className="text-xs font-semibold tracking-wider uppercase text-blue-400 mb-1">Painel administrativo</p>
