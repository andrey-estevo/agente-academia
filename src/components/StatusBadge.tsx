...
export function StatusDot({ status }: { status?: string }) {
  const config = STATUS_CONFIG[(status as Status) || "aguardando"] || STATUS_CONFIG.aguardando;

  return (
    <span className={cn("w-2 h-2 rounded-full inline-block shadow-sm shrink-0", config.dotClass)} />
  );
}
