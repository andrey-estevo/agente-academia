...
const Dashboard = () => {
  ...
  const loadConversations = async () => {
    try {
      ...
      const mapped: Conversation[] = data.map((c: any) => ({
        id: String(c.conversa_id),
        cliente_nome: c.nome,
        cliente_numero: String(c.numero),
        ultima_mensagem: c.ultima_mensagem,
        horario: c.ultima_atualizacao,
        status: c.status,
        setor: c.setor,
      ...
      if (selectedConv) {
        const updated = mapped.find((c) => c.id === selectedConv.id);
  ...
  const handleStatusChange = (convId: string, status: ConversationStatus) => {
    ...
    setSelectedConv((prev) =>
      prev?.id === convId ? { ...prev, status } : prev
    );
  };

  const counts = {
    aguardando: conversations.filter((c) => c.status === "aguardando").length,
    em_atendimento: conversations.filter(
      ...
      <div className="flex-1 flex h-full overflow-hidden">
        ...
        <div className="w-full md:w-[360px] border-r flex flex-col bg-card">
          ...
          <div className="px-4 py-3 border-b flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold flex-1">Conversas</h2>
